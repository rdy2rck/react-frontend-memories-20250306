...
import './style.css';
import { MemoryCard } from '../../types/interfaces';
import { useCookies } from 'react-cookie';
import { ACCESS_TOKEN, MEMORY_DESCRIPTION } from '../../constants';
import { PostMemoryRequestDto } from '../../apis/dto/request/test';
import { postMemoryRequest } from '../../apis';
import { ResponseDto } from '../../apis/dto/response';
import { useNavigate } from 'react-router';
import { MEMORY_TEST_COMPLETE_ABSOLUTE_PATH } from './../../constants/index';
import { useMemoryTestStore } from '../../store';

// interface: 메모리 카드 검사 컴포넌트 속성
interface CardProps {
...
export default function MemoryTest() {
    ...
    if (newSelectedCards.length === 2) {
      const [first, second] = newSelectedCards;
        ...
        setTimeout(() => {
          ...
          setSelectedCards([]);
        }, 500); // 선택한 카드 2개의 색이 일치하지 않을 경우 다시 뒤집음 & 색이 일치할 경우 유지
    }
  };
  ...
  // effect: 검사 카드 리스트가 변경될 시 실행할 함수 //
  useEffect(() => {
    if (startTime && memoryCards.length && memoryCards.every(card => !card.isReverse)) {
      setFinish(true); // 테스트 완료
      const measurementTime = Math.floor((Date.now() - startTime) / 1000); // 테스트 걸린 시간 밀리초에서 초 단위로 변경
      setMeasurementTime(measurementTime); // 테스트에 걸린 시간 측정
    }
  }, [memoryCards]);
